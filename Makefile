...
KIND_CLUSTER ?= maxicloud-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name maxicloud-builder
	$(CONTAINER_TOOL) buildx use maxicloud-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm maxicloud-builder
	rm Dockerfile.cross
...
.PHONY: release
# make manifest, generate docker-build docker-push, deploy kubectl rollout restart までやるやつ
release: manifests generate deploy buf-generate docker-build docker-push
...
HELM_NAMESPACE ?= maxicloud-system
## Name of the Helm release
HELM_RELEASE ?= maxicloud
