...
export const useProjectDetailView = (projectId: string) => {
	const { deployments, project, applications, userByID } =
	...
	const projectDeployments = useMemo(() => {
		const applicationIDSet = new Set(
			projectApplications.map((application) => application.id),
		);
		return deployments.filter((deployment) =>
			applicationIDSet.has(deployment.applicationId),
		);
	...
	const ownerName = project
		? (userByID[project.ownerId]?.displayName ?? "-")
		: "-";
